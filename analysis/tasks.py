...
from analysis.hot_topics import generate_hot_topics_report
...
async def _generate_hot_topics() -> str:
    pool = await create_db_pool()
    try:
        return await generate_hot_topics_report(pool)
    finally:
        await close_db_pool(pool)
...
@celery_app.task(name="analysis.generate_hot_topics_report")
def generate_hot_topics_report_task() -> str:
    return asyncio.run(_generate_hot_topics())
